...
\maketitle
\begin{abstract}
We present an investigation into numerical algorithms for solving complex systems of ODE's, by simulating a many-body problem - our solar system, with varying levels of complexity. We present the forward Euler algorithm and the Velocity-Verlet algorithm, and show that, when computation time is not an enormous concern, the Velocity-Verlet algorithm is better in most cases. We also investigate the appropriate time step to model our system over long time scales, and find that $10^5$ steps per year gives us our required numerical precision. Finally, we present some checks for the consistency of our results, such as conservation of energy/angular momentum and the perihelion precession of mercury predicted by general relativity.
\end{abstract}
...
%WHY THE LINEAR STRUCTURE OF EULER ERROR
%Investigate three-body
%Why periodic energy?
%Do again with taking angular momentum relative to COM?s
\end{document}
