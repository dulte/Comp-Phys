...
folder='/home/gunnnar/Documents/FYS3150/Comp-Phys/Project4/Projec4_Gunnar/L2/N4e7/'
L = 2
...
print "Cv", data[0,3]
print "Cv_a", Cv[0]/(1.0*L*L)
print 'err', lattice.compute_relative_error(data[0,3], Cv[0]/(1.0*L*L))

print "E", data[0,1]
print "E_a", mean_energy[0]/(1.0*L*L)
print 'err', lattice.compute_relative_error(data[0,1], mean_energy[0]/(1.0*L*L))

print "Mean_mag", data[0,-2]
print "Mean_mag_a", mean_magnetic_moment[0]
print 'err', lattice.compute_relative_error(data[0,-2], mean_magnetic_moment[0])

print "Chi", data[0,-1]
print "Chi_a", chi[0]/(1.0*L*L)
print 'err', lattice.compute_relative_error(data[0,-1], chi[0]/(1.0*L*L))
...
Cv=lattice.compute_CV()
plt.plot(data[:,0], Cv/(1.0*L*L))
plt.legend(['Numeric','Analytic'])
#plt.plot(data[np.argmax(data[:,4]),0],np.max(data[:,4]),"ro")
#plt.text(data[np.argmax(data[:,4]),0]-0.01,np.max(data[:,4])+0.1,"$T_c = %g$" %data[np.argmax(data[:,4]),0] )
plt.show()
