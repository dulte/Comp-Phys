...
\begin{table}[!hb]
\centering
\caption{Number of  possible microstates in a simple $2 \times 2$ lattice}
\begin{tabular}{|c|c|}
\hline
...
\begin{figure}[!ht]
    ...
    \end{subfigure}
    \caption{Comparison of the numeric and analytic solution of heat capacity and mean energy per spin for the $2\times 2$ lattice. We plot multiple Monte Carlo steps, $N$, to find where the correspondence becomes good. This was done with temperatures in the interval $\mathrm{kT/J}=2$ to $\mathrm{kT/J}=3$.}\label{fig:2x2_nsteps}
\end{figure}
\linebreak
We also compare explicitly the value from the Ising model and the analytic solution at $\mathrm{kT/J}=1$ in table \ref{tab:2x2_compare} below.
\newpage
\begin{table}[!ht]
\centering
\caption{Explicit comparison for the $2 \times 2$ lattice at the point $\mathrm{kT/J}=1$ for different Monte Carlo cycles, $N$.}\label{tab:2x2_compare}
\bgroup
...
\end{figure}
\newpage
\subsection{Results from our investigation into the time required to reach equilibrium for larger system}
We choose to use a larger lattice, of $20 \times 20$ spins, to investigate the time (measured in number of sweeps of the lattice) that it takes our system to achieve equilibrium. We investigate this first with a low temperature (letting $\mathrm{kT/J}=1.0$). We look at how this system develops if we start with an ordered initial state (all spins pointing up) and if we start with a disordered (uniformly distributed) state. A plot of the mean energy and the mean absolute value of the magnetization in this case is shown below:\\
\begin{figure}[!ht]
    \centering
    \begin{subfigure}[H!]{0.5\textwidth}
        \centering
    ...
    \end{subfigure}
      \caption{Time development of mean energy and mean magic moment per spin for two different initial configurations.Here $L=20$ and $\mathrm{kT/J}=1.0$.}\label{fig:20x20_Sweep_T_1}
\end{figure}
\linebreak
We also plot the same quantities for a temperature of $\mathrm{kT/J}=2.4$ in the figure below:
\newpage
\clearpage
\begin{figure}[!ht]
    \centering
    ...
    \begin{subfigure}[H!]{0.5\textwidth}
        ...
        \caption{Mean absolute magnetic moment per spin from uniform state}
    \end{subfigure}
      \caption{Time development of mean energy and mean magic moment per spin for two different initial configurations. Here $L=20$ and $\mathrm{kT/J}=2.4$.}\label{fig:20x20_Sweep_T_24}
\end{figure}
...
\begin{figure}[!ht]
    ...
    \begin{subfigure}[H!]{0.5\textwidth}
        ...
        \caption{With disordered initial state, at $\mathrm{kT/J}=1$}
    \end{subfigure}%
    ~ 
    \begin{subfigure}[H!]{0.5\textwidth}
        \centering
        \includegraphics[height=2.2in]{flipsWUpStart.png}
        \caption{With uniform initial state, at $\mathrm{kT/J}=1$}
    \end{subfigure}
        ~
     \begin{subfigure}[H!]{0.5\textwidth}
        \centering
        \includegraphics[height=2.2in]{flipsWRandomStartT24.png}
        \caption{With disordered initial state, at $\mathrm{kT/J}=2.4$}
    \end{subfigure}%
    ~ 
    \begin{subfigure}[H!]{0.5\textwidth}
        \centering
        \includegraphics[height=2.2in]{flipsWUpStartT24.png}
        \caption{With uniform initial state, at $\mathrm{kT/J}=2.4$}
    \end{subfigure}
...
\begin{figure}[!ht]
    ...
    \end{subfigure}
       \caption{Time development of mean energy, mean magnetic moment and mean number of accepted flips per spin per Monte Carlo cycle for a system with $L=20$, $\mathrm{kT/J}=1$, with all spins initially pointing up.}\label{fig:20x20_local_min}
\end{figure}
...
\begin{figure}[!ht]
    ...
    \begin{subfigure}[H!]{0.5\textwidth}
        ...
        \caption{$\mathrm{kT/J}=1$}
    \end{subfigure}%
    ~ 
    \begin{subfigure}[H!]{0.5\textwidth}
        \centering
        \includegraphics[height=2.2in]{energyDistHisto.png}
        \caption{$\mathrm{kT/J}=2.4$}
    \end{subfigure}
       \caption{Histogram showing the probability of different energies per spin occurring for two different temperatures. This is for a lattice with $L=20$, simulated with $N=10^6$ Monte Carlo cycles, excluding an equilibration time of 5000 cycles. }\label{fig:histograms}
\end{figure}\\
\linebreak
We also compute the variance of the energy per spin, simply by computing:
$$\mathrm{Var}(E)=\langle E^2 \rangle-\langle E \rangle^2 $$
Where $E$ is the energy \textit{per spin}. Taking the average over the last 100 points (where we should be at equilibrium) gives:
\begin{table}[!hb]
\centering
...
\begin{tabular}{|c|c|}
\hline
Temperature [$\mathrm{kT/J}$] & Variance of energy per spin [$\mathrm{J}^2$], $\sigma^2$\\
\hline
1.0 &  $9.95\times 10^{-5}$\\
2.4 & 0.0203\\
\hline	
...
\linebreak
We have also shown the variance in table \ref{tab:variance_energy}. This is also in line with our physical intuition - the variance is larger at higher temperatures. Note that the distribution is not really Gaussian, and therefore, the variance does not quite characterize the distribution. However, as shown in figure \ref{fig:histograms}, the distribution is \textit{almost} Gaussian for the case where $\mathrm{kT/J}=2.4$. In this case, the standard deviation is:
$$\sigma = \sqrt{0.0203}\approx 0.142$$
Which seems to correspond well with the distribution that we observe in figure \ref{fig:histograms}.
\subsection{The phase transitions}\label{discussion_phase_transition}
Figures \ref{fig:phase_tranition_energy}, \ref{fig:phase_mag_mom}, \ref{fig:phase_Cv} and \ref{fig:phase_chi} show the behavior of our system as a function of temperature. Note, first of all, that the scales on the axis is not quite the same. We began (for $L=40$ and $L=60$), with points in the interval $[2\mathrm{kT/J}, 2.4\mathrm{kT/J}]$, with $0.025 \mathrm{kT/J}$ between each point. However, we realized that the interesting physics (the phase transition) occurred at around $2.3 \mathrm{kT/J}$. To save time, we therefore decided to simulate only until $T=2.35 \mathrm{kT/J}$, with steps of $0.04$ for the $L=100$ case. After considering the data from $L=100$, we decided that it would be sufficient to simulate $L=140$ up to a temperature of $2.3 \mathrm{kT/J}$, with steps of 0.03 $\mathrm{kT/J}$. Thus the odd axis are a result of limited computation time.\\
\linebreak
...
The exact analytic expression can be found in \cite{Project}, and is given by:
